...
func (vs *VSphere) GetPlacementCompatibilityResult(ctx context.Context, pbmClient *pbm.Client, storagePolicyID string) (pbm.PlacementCompatibilityResult, error) {
	...
	if err != nil {
		glog.V(1).Infof("balu - [GetPlacementCompatibilityResult] CheckRequirements failed with err: %+v", err)
		return nil, err
	}
	glog.V(1).Infof("balu - [GetPlacementCompatibilityResult] CheckRequirements result: %+v", res)
	return res, nil
}
...
func (vs *VSphere) getAllAccessibleDatastores(ctx context.Context, vmMo mo.VirtualMachine) ([]string, error) {
	...
	glog.V(1).Infof("balu - [getAllAccessibleDatastores] vmregex for virtual machine is: %+q", vmRegex)
	vmObj, err := f.VirtualMachine(ctx, vmRegex)
	if err != nil {
		glog.V(1).Infof("balu - [getAllAccessibleDatastores] get virtual machine failed with err: %+v", err)
		return nil, err
	}
